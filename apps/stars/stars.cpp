...
extern "C" void main(int argc, char** argv) {
  auto [layer_id, err_openwin]
      = SyscallOpenWindow(kWidth + 4, kHeight + 29, 10, 10, "stars");
  if (err_openwin) {
    exit(err_openwin);
  }

  SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW, 2, 27, kWidth, kHeight, 0x000000);
  ...
  for (int i = 0; i < num_stars; i++) {
    ...
    SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW, 2 + x, 27 + y, 2, 2, 0xfff100);
  }

  SyscallWinRedraw(layer_id);

  auto [tick_end, _] = SyscallGetCurrentTick();

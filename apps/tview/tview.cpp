...
uint64_t OpenTextWindow(int w, int h, const char* title){
  SyscallResult res = SyscallOpenWindow(kWindowMargin*2 + 8*w,
      kWindowTitleHeight + kWindowMargin*2 + 16*h, 10, 10, title);
  if(res.error){
    fprintf(stderr, "%s\n", strerror(res.error));
    exit(1);
  }
...
void DrawLines(const LinesType& lines, int start_line,
    uint64_t layer_id, int w, int height_rows, int tab){
  char buf[1024];
  
  //clear
  SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW, kWindowMargin, kWindowTitleHeight + kWindowMargin,
      8*w, 16*height_rows, 0xffffff);
  ...
  for(int i = 0; i < height_rows; i++){
    ...
    const auto [pline, line_len] = lines[line_index];
    CopyUTF8String(buf, sizeof(buf), pline, line_len, w, tab);
    SyscallWinWriteString(layer_id | LAYER_NO_REDRAW, kWindowMargin, kWindowTitleHeight + 16*i, 0x000000, buf);
  }
...
bool UpdateStartLine(int* start_line, int height_rows, size_t num_lines) {
  while(true){
    const auto [quit, keycode] = WaitEvent();
    if(quit){
      return quit;
    }
    if(num_lines < height_rows){
      continue;
    }
    ...
    if((diff < 0 && *start_line == 0) ||
        (diff > 0 && *start_line == num_lines - height_rows)){
      continue;
    }
    ...
    *start_line += diff;
    if(*start_line < 0){
      *start_line = 0;
    }else if (*start_line > num_lines - height_rows){
      *start_line = num_lines - height_rows;
    }
    return false;
  }
}
...
extern "C" void main(int argc, char** argv){
  ...
  int opt;
  int width = 80, height_rows = 20, tab = 8;
  while((opt = getopt(argc, argv, "w:h:t:")) != -1){
    switch(opt){
    case 'w': width = atoi(optarg); break;
    case 'h': height_rows = atoi(optarg); break;
  ...
  const auto layer_id = OpenTextWindow(width, height_rows, filename);

  const auto lines = FindLines(content, filesize);

  int start_line = 0;
  while(true){
    DrawLines(lines, start_line, layer_id, width, height_rows, tab);
    if (UpdateStartLine(&start_line, height_rows, lines.size())) {
      break;
    }
  }

...
void SetIDTEntry(InterruptDescriptor& desc,
                 ...
                 uint8_t interrupt_stack_table,bool present){                   
  desc.attr.interrupt_stack_table = interrupt_stack_table;
  desc.attr.type = type;
  desc.attr.descriptor_privilege_level = descriptor_privilege_level;
  desc.attr.present = present;
...
namespace {
  // ArrayQueue<Message>* main_msg_queue;
  // std::deque<Message>* main_msg_queue;

  __attribute__((interrupt))
  void IntHandlerXHCI(InterruptFrame* frame){
    Log(kDebugMass, "IntHandlerXHCI\n");
    // main_msg_queue->Push(Message{Message::kInterruptXHCI});
    // main_msg_queue->push_back(Message{Message::kInterruptXHCI});
  ...
  //   // main_msg_queue->push_back(Message{Message::kInterruptLAPICTimer});
  ...
  void PrintHex(uint64_t value, int width, Vector2D<int> pos) {
    for (int i = 0; i < width; ++i) {
      ...
      WriteAscii(*screen_pixel_writer, pos + Vector2D<int>{8 * i, 0}, x, {0, 0, 0});
    }
  }

  void PrintFrame(InterruptFrame* frame, const char* exp_name) {
    WriteString(*screen_pixel_writer, {500, 16*0}, exp_name, {0, 0, 0});
    WriteString(*screen_pixel_writer, {500, 16*1}, "CS:RIP", {0, 0, 0});
    PrintHex(frame->cs, 4, {500 + 8*7, 16*1});
    PrintHex(frame->rip, 16, {500 + 8*12, 16*1});
    WriteString(*screen_pixel_writer, {500, 16*2}, "RFLAGS", {0, 0, 0});
    PrintHex(frame->rflags, 16, {500 + 8*7, 16*2});
    WriteString(*screen_pixel_writer, {500, 16*3}, "SS:RSP", {0, 0, 0});
  ...
  void IntHandlerPF(InterruptFrame* frame, uint64_t error_code) {
    ...
    KillApp(frame);
    PrintFrame(frame, "#PF");
    WriteString(*screen_pixel_writer, {500, 16*4}, "ERR", {0, 0, 0});
...
//   ::main_msg_queue = msg_queue;
void InitializeInterrupt() {

  //load idt
  //const uint16_t kernel_cs = GetCS();
  //Log(kDebug, "cs: %d\n", kernel_cs);
  SetIDTEntry(idt[InterruptVector::kXHCI],
              DescriptorType::kInterruptGate, 0,
  ...
  Log(kDebug, "idt: %08lx\n", reinterpret_cast<uintptr_t>(&idt[InterruptVector::kXHCI]));

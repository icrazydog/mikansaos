...
; 64bits reg:RAX, RBX, RCX, RDX, RSI, RDI, RSP, RBP, R8~R15

bits 64
section .text

global IoOut32  ; void IoOut32(unit16_t addr, uint32_t data);
IoOut32:
  mov dx, di    ; dx = addr
...
global IoIn32  ; uint32_t IoIn32(uint16_addr);
IoIn32:
  mov dx, di  ; dx = addr
  in eax, dx
  ret
...
; set main stack
extern kernel_main_stack_base
extern KernelMainNewStack

global KernelMain
KernelMain:
    mov rsp, qword [kernel_main_stack_base]
...
SwitchContext:  ; void SwitchContext(void* next_ctx(rdi), void* current_ctx(rsi));
    mov [rsi + 0x40], rax
    mov [rsi + 0x48], rbx
    mov [rsi + 0x50], rcx
    mov [rsi + 0x58], rdx
...
RestoreContext:  ; void RestoreContext(void* ctx);
    ; iret stack frame
...
CallApp:  ; int CallApp(int argc, char** argv, uint16_t ss,
    ...
    push r14
    push r15
    mov [r9], rsp ; save to os_stack_ptr

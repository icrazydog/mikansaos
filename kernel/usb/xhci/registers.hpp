...
namespace usb::xhci {
 ...
 union HCCPARAMS2_Bitmap{
    ...
    struct{ 
      ...
      uint32_t virtualization_based_trusted_io_capability :1;
      uint32_t  :22;
    }__attribute__((packed)) bits;
  }__attribute__((packed));
  ...
  union CONFIG_Bitmap{
    uint32_t data[1];
    struct{
      uint64_t max_device_slots_enabled :8;
      uint64_t u3_entry_enable :1;
      uint64_t configuartion_information_enable :1;
      uint64_t  :22;
    }__attribute__((packed)) bits;
  }__attribute__((packed));
...
union IMAN_Bitmap {
  ...
  union IMOD_Bitmap {
    uint32_t data[1];
    struct {
      //250ns increment default 4000 = 1 ms
  ...
  class DoorbellRegister{
    private:
      MemMapRegister<Doorbell_Bitmap> _doorbell;

    public:
      void Ring(uint8_t target, uint16_t steam_id = 0){
        Doorbell_Bitmap value{};
        value.bits.db_target = target;
        value.bits.db_stream_id = steam_id;
        _doorbell.Write(value);
      }
  };
  ...
  struct PortRegisterSet{
    ...
    MemMapRegister<PORTLI_Bitmap> PORTLI_;

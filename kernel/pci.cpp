...
namespace{
  ...
  uint32_t MakeAddress(uint8_t bus, uint8_t device, uint8_t function,
    ...
    return shl(1, 31)
      ...
      //10:8 function 0-7
      | shl(function, 8)
      //register address offset 0-255 unit by 4 byte(reg_addr%4==0)
      //0b1111 1100
      | (reg_addr & 0xfcu);
  }
  ...
  /**
   * add device and check pci bridge
   */
  Error ScanFunction(uint8_t bus, uint8_t device, uint8_t function){
    ...
    if(class_code.Match(0x06u, 0x004u)){
...
namespace pci{
  ...
  //read base address register(0-5)
  WithError<uint64_t> ReadBar(Device& device, unsigned int bar_index){
    if(bar_index > 5){
      return {0 , MAKE_ERROR(Error::kIndexOutOfRange)};
    }

    const auto addr = GetBarAddress(bar_index);
  ...
  Error ConfigureMSIFixedDestination(
    ...
    while (cap_addr != 0) {
      ...
      header.data = pci::ReadConfReg(dev, cap_addr);

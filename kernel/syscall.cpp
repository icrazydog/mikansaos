...
namespace syscall {
  ...
  SYSCALL(OpenWindow) {
    ...
    const auto win = std::make_shared<ToplevelWindow>(
        w, h, screen_frame_buffer_config.pixel_format, title);
  ...
  namespace {
    ...
    Result DoWinFunc(Func f, uint64_t flags_and_layer_id, Args... args) {
    const uint32_t layer_flags = flags_and_layer_id >> 32;
    const unsigned int layer_id = flags_and_layer_id & 0xffffffff;
  ...
  namespace {
    ...
    std::pair<fat32::DirectoryEntry*, int> CreateFile(const char* path) {
      auto [file, err] = fat32::CreateFile(path);
      switch (err.Cause()) {
  ...
  SYSCALL(OpenFile) {
    ...
    auto [file, post_slash] = fat32::FindFile(path);
    if (file == nullptr) {
      if ((flags & O_CREAT) == 0) {
        return { 0, ENOENT };
      }
    ...
    task.Files()[fd] = std::make_unique<fat32::FileDescriptor>(*file);
    return { fd, 0 };
  }
...
extern "C" std::array syscall_table{
  syscall::LogString,/* 0x00 */ 
  syscall::PutString,/* 0x01 */
  syscall::Exit,/* 0x02 */
  syscall::OpenWindow,/* 0x03 */
  syscall::WinWriteString,/* 0x04 */
  syscall::WinFillRectangle,/* 0x05 */
  syscall::GetCurrentTick,/* 0x06 */
  syscall::WinRedraw,/* 0x07 */
  syscall::WinDrawLine,/* 0x08 */
  syscall::CloseWindow,/* 0x09 */
  syscall::ReadEvent,/* 0x0a */
  syscall::CreateTimer,/* 0x0b */
  syscall::OpenFile,/* 0x0c */
  syscall::ReadFile,/* 0x0d */
  syscall::DemandPages,/* 0x0e */
  syscall::MapFile,/* 0x0f */
};

void InitializeSyscall() {
  //bit 0 SCE syscall enable
  //bit 8,10 LME,LMA for 64bit mode
